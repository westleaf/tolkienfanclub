...
def generate_page(from_path, template_path, dest_path, base_path="./"):
    ...
    # Replace href and src paths that start with /
    if base_path:
...
def generate_page_recursive(from_path, template_path, dest_path, base_path="./"):
    if os.path.isdir(from_path):
        for entry in os.listdir(from_path):
            ...
            if os.path.isdir(entry_from_path):
                generate_page_recursive(entry_from_path, template_path, entry_dest_path, base_path)
            elif entry.endswith(".md"):
                entry_dest_path = entry_dest_path[:-3] + ".html"
                generate_page(entry_from_path, template_path, entry_dest_path, base_path)
